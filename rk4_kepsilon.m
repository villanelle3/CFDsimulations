function [u_hat, v_hat] = rk4_kepsilon(u_hat, v_hat, k_hat, eps_hat, mut, kx, ky, k2, nx, ny, Re, dt)
    % RK4 time integration

    % Step 1: Compute k1
    [u_k1, v_k1] = compute_derivatives(u_hat, v_hat, k_hat, eps_hat, mut, kx, ky, k2, nx, ny, Re);
    % [u_k1, v_k1, k_k1, epsilon_k1] = compute_derivatives(u_hat, v_hat, k_hat, epsilon_hat);

    % Step 2: Compute k2
    [u_k2, v_k2] = compute_derivatives(u_hat + 0.5*dt*u_k1, v_hat + 0.5*dt*v_k1, k_hat, eps_hat, mut, kx, ky, k2, nx, ny, Re);
    ...
    % Step 3: Compute k3
    [u_k3, v_k3] = compute_derivatives(u_hat + 0.5*dt*u_k2, v_hat + 0.5*dt*v_k2, k_hat, eps_hat, mut, kx, ky, k2, nx, ny, Re);
    ...
    % Step 4: Compute k4
    [u_k4, v_k4] = compute_derivatives(u_hat + dt*u_k3, v_hat + dt*v_k3, k_hat, eps_hat, mut, kx, ky, k2, nx, ny, Re);
    % [u_k4, v_k4, k_k4, epsilon_k4] = compute_derivatives(u_hat + dt*u_k3, v_hat + dt*v_k3, ...
    %                                                      k_hat + dt*k_k3, epsilon_hat + dt*epsilon_k3);
    
     % Aplica as condições de contorno para garantir velocidade zero nas paredes superior e inferior
